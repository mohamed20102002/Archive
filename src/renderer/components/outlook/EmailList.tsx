    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 h-full flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Select a folder to view emails</p>
        </div>
      </div>
    )
  }
        ...
        {emails.map((email) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left p-3 border-b border-gray-100 dark:border-gray-700 transition-colors ${
                isSelected
                  ? 'bg-primary-50 dark:bg-primary-900/30 border-l-2 border-l-primary-600'
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                <div className="flex items-center gap-1.5 min-w-0">
                  {isArchived && (
                    <span className="flex-shrink-0 text-green-600 dark:text-green-400" title="Archived to topic">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" />
                      </svg>
                    </span>
                  )}
                  <span className={`text-sm truncate ${isSelected ? 'text-primary-900 dark:text-primary-200 font-medium' : 'text-gray-900 dark:text-gray-100'}`}>
              ...
              <h4 className={`text-sm truncate mb-1 ${isSelected ? 'text-primary-800 dark:text-primary-300' : 'text-gray-700 dark:text-gray-300'}`}>
                {email.subject}
              </h4>
